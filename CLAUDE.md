...
- Creating the remote repo (`hosting.rs`, GitHub only) is the one thing git cannot do, so it is the only place Chamber calls a host API. Its credential ladder: a token in our keystore → one git's helpers already hold (`credential_fill`) → OAuth **device flow** (GitHub has no PKCE, so an auth-code flow would need a secret we cannot keep; client id `Ov23liZHFNdT8PInCc9l` is public and compiled in, override with `CHAMBER_GITHUB_CLIENT_ID`) → pasted PAT. GCM is one source of rung 2, never required. Bitbucket/Azure DevOps have no device flow — they use the "I have a URL" tab, and pushing is unaffected by any of this. Identifying the user only ever happens on an explicit Connect, never on dialog open.
...
Not yet verified end to end on a real remote. v0.1.0 shipped 2026-09-06 (signed, notarized). Releases auto-publish; in-app updater (tauri-plugin-updater, `UpdatesService`, sidebar pill) from 0.1.1. `/release` skill cuts one. Open items: Windows/Linux smoke test, encrypted filenames (optional), real-device test of QR scan + GCM sign-in.
