...
export function vaultPageTemplate(self: VaultPage) {
	...
	return html`
		<aside>
			...
			<nav>
				<div class="nav-item ${self.room === null ? 'active' : ''} ${self.dropRoom === '' && self.dragging ? 'target' : ''}" data-room="" @click=${() => (self.room = null)}>
				...
				${self.rooms.length ? html`<span class="nav-label inscr">Rooms</span>` : ''}
				${self.rooms.map((r) => html`
					<div class="nav-item ${self.room === r.name ? 'active' : ''} ${self.dragging && self.dropRoom === r.name ? 'target' : ''}" data-room=${r.name} @click=${() => (self.room = r.name)}>
						<ml-icon icon="folder" size="sm"></ml-icon><span class="label">${r.name}</span><span class="count">${r.count}</span>
					</div>`)}
		...
		<main>
			<header data-tauri-drag-region>
				...
				${c ? html`
					...
					<ml-button variant="primary" size="sm" ?disabled=${!c.hasAccess} @ml:click=${() => self.addFiles()}><ml-icon slot="icon-start" icon="plus" format="bold"></ml-icon>Add secret</ml-button>
				` : ''}
			</header>
			...
			<section>
				<div class="card">
					${!c ? html`
						...
						<div class="list">
							<div class="list-head">
								<span class="title">${self.room === null ? 'All secrets' : self.room === 'recent' ? 'Recently sealed' : self.room}</span>
								<span class="count">${self.filtered.length}</span>
								<span class="grow"></span>
								<div class="filters">
							...
							<div class="rows">
								${self.filtered.length === 0 ? html`
									<div class="empty" style="height: 100%">
										...
										<div class="big">${self.entries.length === 0 ? 'Nothing sealed yet' : 'No matches'}</div>
										<div>${self.entries.length === 0 ? 'Drop files anywhere in this window, or use Add secret.' : 'Try a different search or room.'}</div>
									</div>` : ''}
								${self.filtered.map((e) => html`
									<div class="row ${self.selectedPath === e.path ? 'selected' : ''}" @click=${() => self.open(e.path)}>
							...
							<div class="list-foot">
								<span>${self.entries.length} secret${self.entries.length === 1 ? '' : 's'} · ${self.rooms.length} room${self.rooms.length === 1 ? '' : 's'}</span>
								<span style="display:inline-flex; align-items:center; gap:6px"><ml-icon icon="upload-simple" size="xs"></ml-icon>Drop files to add</span>
							</div>
						</div>
			...
			${self.dragging ? html`
				<div class="drop"><div class="drop-inner">
					...
					<div class="fine">Drop on a room in the sidebar to seal it there instead</div>
				</div></div>` : ''}
		</main>

		<!-- dialogs -->
		<ml-dialog id="new-chamber" size="sm">
